...
@Component
public class verifyExistOneCharUpper implements IValidatePassword {
    @Override
    public void valid(String password) {
        Pattern pattern = Pattern.compile("[A-Z]");
        Matcher matcher = pattern.matcher(password);
        if (!matcher.find()){
            throw new PasswordValidationException(USER_PASSWORD_VALID.params("At least one uppercase letter").getMassage());
        }
    }
}